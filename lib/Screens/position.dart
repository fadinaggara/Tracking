...
class _OSM1State extends State<OSM1> {
  ...
  Future<String> _getAdressFromLatLng(double? pLat, double? pLng) async {
    ...
    localAdress = '${street}, ${locality}, ${administrativeArea}, ${postalCode}';
    return localAdress;
  }
  ...
  void save() {
    saveLastMarkerToFb(gpsLat.toString(), gpsLng.toString()); // SAVE DEST MARKER (blue)
  ...
  void saveLastMarkerToFb(String lat, String lng) async {
    DatabaseReference ref = FirebaseDatabase.instance.ref("/allData/destinations");
    DateTime now = DateTime.now();
    String time = (now.hour.toString() + ":" + now.minute.toString());
    String localadress = await _getAdressFromLatLng(double.parse(lat), double.parse(lng));
  ...
  Widget lastLocationPopupDialog(BuildContext context) {
    final destinationsRef = FirebaseDatabase.instance.ref('/allData/destinations');
    return AlertDialog(
                      ...
                      trailing: IconButton(
                        onPressed: () {
                          FirebaseDatabase.instance
                              .ref('/allData/destinations/${snapshot.key}')
                              .remove();
